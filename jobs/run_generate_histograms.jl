...
if ENV_name == "gwdg" || ENV_name == "owl"
  ...
  function generate_histogram_set_ppi(ppi::Int64=10; K::Int64=38, N::Int64=32, incident_10p::Int64=550)
    ...
    for img in images
      if img <= images[4]
        generate_histograms(; max_pictures = img, max_triplets = Integer(0), Ncores=8, N=N, photons_per_image=ppi, batchsize = Integer(img/8), successive_jobs=1, prefix="SH_", suffix="", use_cube=false, qcut_ratio=1.0, K=K, rmax=float(K), histogram_method="histogramCorrelationsInPicture_alltoall")

      else
        numbersets = ceil(Int64, img / setsize)
        for i = 1:numbersets
          generate_histograms(; max_pictures = setsize, max_triplets = Integer(0), Ncores=8, N=N, photons_per_image=ppi, batchsize = Integer(setsize/8), successive_jobs=1, prefix="parts/SH_", suffix="_$(i)", use_cube=false, qcut_ratio=1.0, K=K, rmax=float(K), histogram_method="histogramCorrelationsInPicture_alltoall")
        end
      end
    end
  end
